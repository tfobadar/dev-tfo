...
export default function ScheduleCallForm({ blok }: StoryblokComponentProps) {
      ...
      buttonsProps: [
        {
          ...
          onClick: () => {
            if (!isSubmitted) {
              trackFormErrorSubmitEvent({
                label: `${'Request a call-' + document.title}`,
              });
            }
          },
        },
  ...
  const submit = async (values: INPLcallFormValues) => {
    ...
    data.append('meeting_type', values.meetingType);
    data.append('additional_information__optional_', values.message);
  ...
  const FORM_VALIDATION_SCHEMA = Yup.object().shape({
    ...
    meetingType: Yup.string().required(
      meetingType?.validators?.[0]?.errorMessage,
    ),
  ...
  const handleDateChange = async (date: Date[]) => {
    ...
    if (response) {
      const availiabilitySlots = response?.data?.AvailabilityView?.Availability;
      const tempSlotsArr = [];
    ...
    <Box textAlign="start">
      {isSubmitted ? (
        ...
        <>
          ...
          <Formik
            initialValues={INPL_SCHEDULE_INITIAL_VALUES}
            onSubmit={submit}
          ...
          >
            {({ errors, touched }) => (
              <Form>
                ...
                <Flex gap={4} flexWrap={{ base: 'wrap', md: 'wrap' }}>
                  <Box position="relative" w={{ base: 'full', md: '100%' }}>
                    <Box
                      as="label"
                      color={theme.colors.gray[500]}
                      fontSize={'sm'}
                  ...
                  <Field name="meetingType">
                    {({ field, form, meta }: FieldProps) => (
                      ...
                      >
                        <Select
                          name={field.name}
                          placeholder={meetingType.placeholder}
