...
const Pagination = ({
  ...
  const pageURL = (router.asPath === '/' ? '' : router.asPath).split('?')[0];
  const lang = router.locale;

  const urlPage = (page: number | string) =>
    ...
    <nav className={styles['next-pagination']} aria-label="pagination">
      <Head>
        ...
        {currentPage !== 1 ? (
          <link
            rel="prev"
            href={`/${lang}/${pageURL}${urlPage(currentPage - 1)}`}
          />
        ) : null}
        {!isLastPage ? (
          <link
          ...
          pageNumber === '...' ? (
            ...
            <Item key={pageNumber} theme={styles}>
              {pageNumber === currentPage ? (
                ...
                <NextLink
                  href={`/${lang}/${pageURL}${urlPage(pageNumber)}`}
                  prefetch={false}
                  passHref
                  legacyBehavior
                  {...linkProps}
